                            ...
                            @if (($taskSettings->comments == 'yes' && in_array('client', user_roles())) || in_array('admin', user_roles()) || in_array('employee', user_roles()))
                                ...
                                @php
                                    ...
                                    $canViewTaskLinks = (
                                        ...
                                        || ($viewTaskLinkPermission == 'added' && $task->added_by == user()->id)
                                        || ($viewTaskLinkPermission == 'owned' && in_array(user()->id, $taskUsers))
                                        || ($viewTaskLinkPermission == 'both' && (in_array(user()->id, $taskUsers) || $task->added_by == user()->id))
                                    );
                                @endphp
                                @if ($canViewTaskLinks)
                                    <x-tab-item class="ajax-tab" :active="(request('view') === 'links')"
                                                :link="route('tasks.show', $task->id).'?view=links'">
                                        @lang('modules.tasks.links')</x-tab-item>
                                @endif
                            @endif
