<!doctype html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ (app()->getLocale() == 'ar') ? 'rtl' : 'ltr' }}" @if(app()->getLocale() == 'ar') class="rtl" @endif>
...
<script>
    ...
    // Intercept Bootstrap Select initialization to automatically apply RTL direction if active
    (function ($) {
        if (typeof $.fn.selectpicker !== 'function') {
            console.warn('Bootstrap Select was not loaded before RTL initialization.');
            return;
        }

        if ($.fn.selectpicker.__rtlWrapped) {
            return;
        }

        const originalSelectpicker = $.fn.selectpicker;

        const rtlAwareSelectpicker = function () {
            const currentDirection =
                document.documentElement.getAttribute('dir') === 'rtl'
                    ? 'rtl'
                    : 'ltr';

            this.attr('dir', currentDirection);

            return originalSelectpicker.apply(this, arguments);
        };

        Object.assign(rtlAwareSelectpicker, originalSelectpicker);
        rtlAwareSelectpicker.__rtlWrapped = true;

        $.fn.selectpicker = rtlAwareSelectpicker;
    })(jQuery);
