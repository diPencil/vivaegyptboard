                        ...
                        />
                            ...
                            @if ($showFullProfile)
                                ...
                                @if(in_array('payroll', user_modules()))
                                    @php
                                        $netSalary = 0;
                                        if (class_exists(\Modules\Payroll\Entities\EmployeeMonthlySalary::class)) {
                                            $salaryInfo = \Modules\Payroll\Entities\EmployeeMonthlySalary::employeeNetSalary($employee->id);
                                            $netSalary = $salaryInfo['netSalary'] ?? 0;
                                            $payrollCurrency = \Modules\Payroll\Entities\PayrollSetting::with('currency')->first();
                                            $salaryCurrencyId = $payrollCurrency && $payrollCurrency->currency ? $payrollCurrency->currency->id : company()->currency->id;
                                        }
                                    @endphp
                                    <x-cards.data-row :label="__('payroll::modules.payroll.netSalary') . ' (' . __('app.monthly') . ')'"
                                        :value="$netSalary > 0 ? currency_format($netSalary, $salaryCurrencyId) : '--'" />
                                @endif
