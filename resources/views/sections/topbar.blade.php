...
<header class="main-header clearfix bg-white" id="header">
    ...
    <div class="page-header-right float-right d-flex align-items-center justify-content-end">
        ...
        <!-- NOTIFICATIONS START -->
            ...
            <!-- NOTIFICATIONS END -->
            <!-- LANGUAGE START -->
            <li title="{{__('app.language')}}">
                <div class="language_box dropdown">
                    <a class="d-block dropdown-toggle header-icon-box" type="link"
                       data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <i class="fa fa-language f-16 text-dark-grey"></i>
                    </a>
                    <!-- DROPDOWN - INFORMATION -->
                    <div class="dropdown-menu dropdown-menu-right" tabindex="0">
                        @foreach($languageSettings as $language)
                            <form method="POST" action="{{ route('settings.change_language') }}">
                                @csrf
                                <input type="hidden" name="lang" value="{{ $language->language_code }}">
                                <button type="submit" class="dropdown-item @if(user()->locale == $language->language_code) active @endif">
                                    <span class="flag-icon flag-icon-{{ strtolower($language->flag_code) }}"></span> {{ $language->language_name }}
                                </button>
                            </form>
                        @endforeach
                    </div>
