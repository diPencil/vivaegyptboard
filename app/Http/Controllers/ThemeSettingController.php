...
class ThemeSettingController extends AccountBaseController
{
    ...
    public function store(UpdateThemeSetting $request)
    {
        ...
        if ($request->has('login_ui')) {
            $setting->login_ui = $request->login_ui;
        }
        ...
        if ($request->login_logo_delete == 'yes') {
            Files::deleteFile($setting->login_logo, GlobalSetting::APP_LOGO_PATH);
            $setting->login_logo = null;
        }

        if ($request->hasFile('login_logo')) {
            Files::deleteFile($setting->login_logo, GlobalSetting::APP_LOGO_PATH);
            $setting->login_logo = Files::uploadLocalOrS3($request->login_logo, GlobalSetting::APP_LOGO_PATH, width: 400);
        }
