...
class TaskController extends AccountBaseController
{
    ...
    use ProjectProgress, TaskAccessTrait;
    ...
    public function show($id)
    {
        ...
        abort_403(!$overrideViewPermission && !$this->verifyTaskAccess($this->task));
        ...
        case 'comments':
            ...
            break;
        case 'links':
            $viewTaskLinkPermission = user()->permission('view_task_links');
            $taskUsers = $this->task->users->pluck('id')->toArray();
            $canViewTaskLinks = (
                $viewTaskLinkPermission == 'all'
                || ($viewTaskLinkPermission == 'added' && $this->task->added_by == user()->id)
                || ($viewTaskLinkPermission == 'owned' && in_array(user()->id, $taskUsers))
                || ($viewTaskLinkPermission == 'both' && (in_array(user()->id, $taskUsers) || $this->task->added_by == user()->id))
            );
            abort_403(!$canViewTaskLinks);
            $this->tab = 'tasks.ajax.links';
            break;
        case 'notes':
